document.addEventListener("DOMContentLoaded", function() {
    ...
    if (fileTreeContainer && contentDisplay) {
        ...
        function createTree(nodes) {
            ...
            nodes.forEach(node => {
                ...
                if (node.type === 'directory') {
                    ...
                    // 创建可点击的文件链接，在 notes.html 中加载到 iframe
                    const fileLink = document.createElement('a');
                    fileLink.className = 'file';
                    fileLink.href = '/articles/' + node.path.replace(/\.md$/, '.html');
                    fileLink.dataset.path = node.path;
                    fileLink.setAttribute('aria-label', 'Open article: ' + node.name.replace('.md', ''));
                    ...
                    fileLink.appendChild(svg);
                    fileLink.appendChild(span);

                    // 添加title属性，鼠标悬停时显示完整文件名
                    fileLink.title = node.name.replace('.md', '');

                    // 添加点击事件，在 iframe 中加载文章
                    fileLink.addEventListener('click', function(e) {
                        e.preventDefault();

                        // 更新URL
                        history.pushState({ articleUrl: fileLink.href }, '', fileLink.href);

                        loadArticleInIframe(fileLink.href);

                        // 更新活动状态
                        const allFiles = fileTreeContainer.querySelectorAll('.file');
        ...
        function loadArticleInIframe(articleUrl, updateActiveLink = true) {
            // 使用 AJAX 加载文章内容并在当前页面显示，保持与 notes.html 相同的界面
            fetch(articleUrl)
                .then(response => {
                    if (!response.ok) throw new Error('Network response was not ok');
                ...
                .then(html => {
                    ...
                    // 提取文章内容 - 更精确地提取，避免嵌套问题
                    const articleContentElement = doc.querySelector('#content-display');
                    ...
                    // 复制所有子节点，避免直接使用 innerHTML 可能导致的问题
                    Array.from(articleContentElement.childNodes).forEach(node => {
                        contentDisplay.appendChild(node.cloneNode(true));
                    });
                    ...
                    // 清理样式，确保正常显示
                    contentDisplay.classList.remove('has-iframe');
                    const mainContentWrapper = contentDisplay.closest('.main-content-wrapper');
                    if (mainContentWrapper) {
                        mainContentWrapper.classList.remove('has-iframe');
                        mainContentWrapper.style.overflow = '';
                    }

                    contentDisplay.style.overflow = '';
                    contentDisplay.style.padding = '';
                    contentDisplay.style.position = '';
                    ...
                    // 更新活动链接状态
                    if (updateActiveLink) {
                        updateActiveLinkByUrl(articleUrl);
                    }
                })
                .catch(error => {
                    ...
                    errorDiv.textContent = '文章加载失败: ' + error.message;
                    contentDisplay.appendChild(errorDiv);
                });
        }

        // 根据URL更新活动链接状态
        function updateActiveLinkByUrl(articleUrl) {
            const allFiles = fileTreeContainer.querySelectorAll('.file');
            allFiles.forEach(f => {
                if (f.href === articleUrl) {
        ...
        window.addEventListener('popstate', function(event) {
            ...
            // 检查是否是文章页面
            if (currentPath.includes('/articles/') && currentPath.endsWith('.html')) {
                loadArticleInIframe(window.location.href, true);
            } else {
                // 返回到笔记列表页面，显示占位符
                while (contentDisplay.firstChild) {
                    contentDisplay.removeChild(contentDisplay.firstChild);
                ...
                placeholderDiv.textContent = '请从左侧选择一篇文章开始阅读';
                contentDisplay.appendChild(placeholderDiv);
        ...
        function handleInitialLoad() {
            // 首先检查是否有从文章页面重定向过来的URL
            const redirectedUrl = sessionStorage.getItem('redirectedArticleUrl');
            if (redirectedUrl) {
                // 清除sessionStorage
                sessionStorage.removeItem('redirectedArticleUrl');

                // 使用 replaceState 更新URL，不添加历史记录
                history.replaceState({ articleUrl: redirectedUrl }, '', redirectedUrl);

                // 加载文章
                loadArticleInIframe(redirectedUrl, true);
                return;
            }

            // 如果没有重定向URL，检查当前URL是否包含文章路径
            const currentPath = window.location.pathname;
            if (currentPath.includes('/articles/') && currentPath.endsWith('.html')) {
                // 构建完整的文章URL
                const articleUrl = window.location.origin + currentPath;
                loadArticleInIframe(articleUrl, true);
            }
        }
            ...
            setTimeout(() => {
                ...
                if (matchedFiles.length > 0) {
                    const resultsHTML = matchedFiles.map(fileData => {
                        const articleUrl = '/articles/' + fileData.path.replace(/\.md$/, '.html');
                        const snippet = createSnippet(fileData.content, searchTerm);
                        return '<div class="search-result-item">' +
                                    '<a href="' + articleUrl + '" class="search-result-link" data-article-url="' + articleUrl + '">' + fileData.displayName + '</a>' +
                    ...
                    // 为搜索结果添加点击事件，在 iframe 中加载
                    const searchResultLinks = contentDisplay.querySelectorAll('.search-result-link');
                    searchResultLinks.forEach(link => {
                        link.addEventListener('click', function(e) {
                            e.preventDefault();
                            const articleUrl = link.getAttribute('data-article-url');

                            // 更新URL
                            history.pushState({ articleUrl: articleUrl }, '', articleUrl);

                            loadArticleInIframe(articleUrl);
                        });
                    });
                } else {
