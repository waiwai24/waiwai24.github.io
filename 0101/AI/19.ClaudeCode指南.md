...
- ai agent 是一个非确定性系统，而多 agent 则是叠加不确定性
- 一个api call 能解决的问题，不要用使用 agent 而用agent
- sub agent 与顶层规划调度agent上下文应该不一样，否则失去了意义



## 5.Skills

Skills 是 claude 动态加载的指令、脚本和资源的文件夹，可以提高在特定任务上的性能，相较于 MCP 在启动时就将全部 tool 描述引入更加节省上下文，它采取渐进式披露策略，claude code 启动时只加载可用技能名称和描述，当任务与技能描述相匹配时，加载全部 Skill.md 到上下文，最终按照指令规则执行
